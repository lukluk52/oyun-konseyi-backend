...
router.get('/public', async (req, res) => {
  try {
    ...
    const discussions = await Discussion.find(filter).sort({ createdAt: -1 }).limit(50);
...
router.post('/public', async (req, res) => {
  try {
    ...
    const discussion = await Discussion.create({ title, content, author: author || 'Anonim', authorId, game: game || 'Genel' });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (role !== 'admin' && discussion.authorId?.toString() !== userId) return res.status(403).json({ message: 'Yetkiniz yok' });
...
router.post('/admin', adminAuth, async (req, res) => {
  try {
    ...
    const discussion = await Discussion.create({ title, content, author: author || 'Admin', game: game || 'Genel' });
    res.status(201).json(discussion);
  } catch (error) {
